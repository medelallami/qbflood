...
const CategoryFilters: FC = observer(() => {
  ...
  if (!SettingStore.floodSettings.UISidebarFilterCategory) {
    return null;
  }

  const categories = Object.keys(TorrentFilterStore.taxonomy.categoryCounts);

  if ((categories.length === 1 && categories[0] === '') || (categories.length === 2 && categories[1] === 'uncategorized')) {
    return null;
  }
  ...
  const filterElements = filterItems.map((filter) => (
    <SidebarFilter
      ...
      count={TorrentFilterStore.taxonomy.categoryCounts[filter] || 0}
      key={filter}
      isActive={
        (filter === '' && !TorrentFilterStore.categoryFilter.length) ||
        TorrentFilterStore.categoryFilter.includes(filter)
      }
      name={filter}
      slug={filter}
      size={TorrentFilterStore.taxonomy.categorySizes[filter]}
    />
  ));
